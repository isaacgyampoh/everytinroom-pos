  ...
  return (<>
    <nav className="hidden md:flex fixed top-0 left-0 right-0 h-[72px] glass border-b border-white/50 px-6 items-center z-[100]">
      ...
      <div className="ml-auto flex items-center gap-4">
        <div className="flex items-center gap-3 py-2 px-4 pl-2 bg-gray-100 rounded-full"><div className="w-[38px] h-[38px] bg-brand-500 rounded-full flex items-center justify-center text-white font-bold">{user?.name?.charAt(0)}</div><span className="text-sm font-semibold">{user?.name}</span></div>
    ...
    <button onClick={onOpenCart} className="fixed bottom-[calc(100px+env(safe-area-inset-bottom))] md:bottom-8 right-5 md:right-8 w-16 h-16 bg-brand-500 rounded-full flex items-center justify-center text-3xl text-white z-[99] shadow-lg shadow-brand-500/40 active:scale-90 transition">🛒{cartCount>0&&<span className="absolute top-0 right-0 min-w-[24px] h-6 bg-red-500 rounded-full text-xs font-bold text-white flex items-center justify-center border-[3px] border-white">{cartCount}</span>}</button>
    <nav className="flex md:hidden fixed bottom-0 left-0 right-0 glass safe-bottom px-4 pt-2.5 z-[100]">{MOBILE_NAV.filter(n=>!n.admin||isAdmin).map(n=>(
      <button key={n.id} onClick={()=>go(n.id)} className={`flex-1 flex flex-col items-center gap-1 py-1.5 rounded-xl text-[10px] font-semibold relative transition ${page===n.id?(n.wa?'text-wa bg-wa/10':'text-brand-500 bg-brand-50'):'text-gray-400'}`}>
    ...
    <div className={`fixed top-0 right-0 bottom-0 w-80 max-w-[85%] bg-white z-[201] flex flex-col transition-transform duration-300 ${drawerOpen?'translate-x-0':'translate-x-full'}`}>
      <div className="flex items-center justify-between p-5 safe-top border-b border-gray-100"><h3 className="text-lg font-bold">Menu</h3><button onClick={()=>setDrawerOpen(false)} className="w-11 h-11 bg-gray-100 rounded-xl flex items-center justify-center text-xl">✕</button></div>
      <div className="flex-1 overflow-y-auto p-5">
        <div className="bg-brand-500 rounded-2xl p-6 text-center text-white mb-6"><div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center text-2xl font-bold mx-auto mb-3">{user?.name?.charAt(0)}</div><h4 className="text-lg font-bold">{user?.name}</h4><span className="inline-block mt-2 px-3.5 py-1.5 bg-white/20 rounded-full text-[11px] font-bold">{user?.role?.toUpperCase()}</span></div>
        <div className="space-y-1.5">{NAV_ITEMS.filter(n=>!n.admin||isAdmin).map(n=>(
          <button key={n.id} onClick={()=>go(n.id)} className={`flex items-center gap-3.5 w-full p-4 rounded-xl text-[15px] font-semibold relative text-left transition ${page===n.id?(n.wa?'bg-wa text-white':'bg-brand-500 text-white'):'text-gray-500 hover:bg-gray-50'}`}>
      ...
      <div className="p-5 safe-bottom border-t border-gray-100"><button onClick={()=>{logout();setDrawerOpen(false)}} className="w-full p-4 bg-red-50 rounded-xl text-[15px] font-semibold text-red-600">🚪 Sign Out</button></div>
    </div>
  </>)
}
