  ...
  return (<>
    ...
    <nav className="hidden md:flex fixed top-0 left-0 right-0 h-16 glass border-b border-stone-200/50 px-5 items-center z-[100]">
      ...
      <div className="flex gap-0.5 ml-5 flex-wrap">{NAV.filter(n=>!n.admin||isAdmin).map(n=>(
        <button key={n.id} onClick={()=>go(n.id)} className={`px-2.5 py-1.5 rounded-xl text-[11px] font-semibold relative transition ${page===n.id ? 'bg-gray-900 text-white' : 'text-stone-400 hover:text-stone-700 hover:bg-stone-100'}`}>
