...
export default function InvoicePay() {
  ...
  // Get order ID from URL hash: /#/pay/ORDER_ID
    ...
    try {
      const res = await fetch('https://noiiuwkovoojkcwzupye.supabase.co/functions/v1/charge-momo?action=initialize', {
        ...
        body: JSON.stringify({
          ...
          metadata: { order_id: order.id, order_no: order.order_no, customer: order.customer_name }
        }),
      })
  ...
  // Check if payment was just completed (callback)
  useEffect(() => {
      ...
      <div className="max-w-lg mx-auto px-6 py-6">
        ...
        <div className="bg-white rounded-2xl p-4 mb-4">
          <div className="flex justify-between text-sm mb-2">
            ...
            <span className="font-semibold">{money(order?.subtotal || order?.total)}</span>
          </div>
          {Number(order?.delivery_fee) > 0 && (
            <div className="flex justify-between text-sm mb-2">
              <span className="text-gray-500">Delivery</span>
              <span className="font-semibold">{money(order?.delivery_fee)}</span>
            </div>
          )}
