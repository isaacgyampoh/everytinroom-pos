      ...
      <div className="bg-white rounded-3xl p-6 shadow-md overflow-x-auto">
        <table className="w-full min-w-[400px]">
          ...
          <tbody>{bundles.length === 0 ? <tr><td colSpan={5} className="text-center py-12 text-gray-400">No bundles</td></tr> : bundles.map(b => {
            ...
            return (<tr key={b.id} className="border-b border-gray-50"><td className="p-3 text-sm font-semibold">{b.name}</td><td className="p-3 text-xs text-gray-500">{names}</td><td className="p-3 text-brand-500 font-bold text-sm">{money(b.bundlePrice)}</td><td className="p-3"><span className={`px-2.5 py-1 rounded-lg text-[11px] font-bold ${b.active ? 'bg-green-50 text-green-500' : 'bg-red-50 text-red-500'}`}>{b.active ? 'Active' : 'Off'}</span></td><td className="p-3"><div className="flex gap-2"><button onClick={() => openEdit(b)} className="h-9 px-3 bg-gray-100 rounded-lg text-xs">✏️</button><button onClick={() => del(b.id)} className="h-9 px-3 bg-red-50 text-red-500 rounded-lg text-xs">🗑️</button></div></td></tr>)
          })}</tbody>
        </table>
      </div>
