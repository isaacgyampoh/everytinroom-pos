        ...
        <div className="overflow-x-auto rtable-wrap">
          <table className="rtable w-full min-w-[400px]">
            ...
            <tbody>{filtered.length === 0 ? <tr><td colSpan={4} className="text-center py-12 text-gray-400">No customers</td></tr> : filtered.map(c => (
              <tr key={c.id} className="border-b border-gray-50"><td data-label="Phone" className="p-3 text-sm font-semibold">{c.phone}</td><td data-label="Visits" className="p-3"><span className="px-2.5 py-1 bg-gray-50 text-gray-600 rounded-lg text-[11px] font-bold">{c.visitCount}</span></td><td data-label="Total Spent" className="p-3 text-sm font-bold text-green-500">{money(c.totalSpent)}</td><td data-label="Last Visit" className="p-3 text-sm">{fmtDate(c.lastVisit)}</td></tr>
            ))}</tbody>
          </table>
        </div>
      </div>
