...
export default function RefundsPage() {
  ...
  return (
    <div >
      ...
      <div className="bg-violet-500 rounded-2xl p-7 text-white mb-6"><small className="text-sm opacity-80">Total Refunds Today</small><strong className="block text-[22px] md:text-[26px] font-bold mt-2">{money(todayTotal)}</strong></div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="↩️ Process Refund"
        ...
        <div className="space-y-4">
          ...
          {sale && (<>
            ...
            <div className="bg-violet-500 rounded-2xl p-6 text-white"><small className="text-sm opacity-80">Refund Amount</small><strong className="block text-[22px] md:text-[26px] font-bold mt-2">{money(refundAmount)}</strong></div>
          </>)}
        </div>
      </Modal>
    </div>
