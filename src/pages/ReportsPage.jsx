...
export default function ReportsPage() {
  ...
  const calc = (arr) => {
    ...
    for (const s of arr) { if (s.voided) continue; total += s.total; profit += s.profit; txn++
      if (s.payment === 'Cash') cash += s.total; else if (s.payment === 'Momo') momo += s.total; else paystack += s.total }
  ...
  const StatCard = ({ icon, label, value, color }) => (
    <div className="bg-white rounded-2xl p-5 shadow-sm"><div className="text-xs font-bold text-gray-400 uppercase mb-1">{icon} {label}</div><div className="text-2xl font-extrabold" style={color ? { color } : {}}>{value}</div></div>
  )

  const Section = ({ title, color, borderColor, data, exp, ref: refAmt }) => (
    <div className="bg-white rounded-3xl p-6 shadow-md mb-5" style={{ borderLeft: `5px solid ${borderColor}` }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard icon="↩️" label="Refunds" value={money(refAmt)} color="#8b5cf6" />
  ...
  return (
    <div className="animate-fade">
      ...
      <Section title="📅 Today" borderColor="#0ea5e9" data={todayData} exp={todayExp} ref={todayRef} />
      <Section title="📆 This Week" borderColor="#22c55e" data={weekData} exp={weekExp} ref={weekRef} />
      <Section title="📊 This Month" borderColor="#f59e0b" data={monthData} exp={monthExp} ref={monthRef} />
      ...
      <div className="bg-white rounded-3xl p-6 shadow-md mb-5" style={{ borderLeft: '5px solid #ef4444' }}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[400px]">
            <thead><tr><th className="p-3 bg-gray-50 text-left text-[11px] font-bold text-gray-500 uppercase">Date</th><th className="p-3 bg-gray-50 text-left text-[11px] font-bold text-gray-500 uppercase">Category</th><th className="p-3 bg-gray-50 text-left text-[11px] font-bold text-gray-500 uppercase">Description</th><th className="p-3 bg-gray-50 text-right text-[11px] font-bold text-gray-500 uppercase">Amount</th></tr></thead>
            <tbody>
              {expenses.filter(e => isoDate(e.date) >= ms).map(e => (
                <tr key={e.id} className="border-b border-gray-50"><td className="p-3 text-sm">{fmtDate(e.date)}</td><td className="p-3"><span className="px-2 py-1 bg-red-50 text-red-500 rounded-lg text-[10px] font-bold">{e.category}</span></td><td className="p-3 text-sm">{e.description}</td><td className="p-3 text-sm text-right font-bold text-red-500">{money(e.amount)}</td></tr>
              ))}
              {expenses.filter(e => isoDate(e.date) >= ms).length === 0 && <tr><td colSpan={4} className="text-center py-6 text-gray-400">No expenses</td></tr>}
            </tbody>
            <tfoot><tr className="bg-gray-50 font-bold"><td colSpan={3} className="p-3 text-sm">Total</td><td className="p-3 text-sm text-right text-red-500">{money(monthExp)}</td></tr></tfoot>
          </table>
        </div>
      </div>
          ...
          <table className="w-full min-w-[400px]">
            ...
            <tbody>
              {refunds.filter(r => isoDate(r.date) >= ms).map(r => (
                <tr key={r.id} className="border-b border-gray-50"><td className="p-3 text-sm">{fmtDate(r.date)}</td><td className="p-3 text-sm font-bold text-violet-500">{r.refundNo}</td><td className="p-3 text-sm">{r.reason}</td><td className="p-3 text-sm text-right font-bold text-violet-500">{money(r.refundAmount)}</td></tr>
              ))}
              {refunds.filter(r => isoDate(r.date) >= ms).length === 0 && <tr><td colSpan={4} className="text-center py-6 text-gray-400">No refunds</td></tr>}
            </tbody>
            <tfoot><tr className="bg-gray-50 font-bold"><td colSpan={3} className="p-3 text-sm">Total</td><td className="p-3 text-sm text-right text-violet-500">{money(monthRef)}</td></tr></tfoot>
          </table>
        </div>
      </div>
    </div>
