...
// Memoized product card - prevents re-render when cart changes
      ...
      <div className="p-3 md:p-3.5">
        ...
        <div className="flex items-end justify-between mt-2">
          <div>
            ...
            <div className={`text-lg font-extrabold leading-none ${hasPromo ? 'text-amber-600' : 'text-gray-900'}`}>{money(price)}</div>
          </div>
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${qty === 0 ? 'bg-red-100 text-red-600' : qty <= 5 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
            {qty === 0 ? 'OUT' : qty}
          </span>
        </div>
      </div>
  ...
  return (
    <div className="animate-fade">
      ...
      <div className="relative mt-5 mb-4">
        ...
        <input className="w-full h-12 md:h-13 pl-12 pr-4 bg-white rounded-2xl text-sm font-medium placeholder:text-stone-300 focus:outline-none focus:ring-2 focus:ring-amber-400/40" placeholder="Search or scan barcode..." value={query}
        ...
        <div className="flex gap-1.5 overflow-x-auto mb-5 scrollbar-hide">
          {categories.map(c => (
            <button key={c} onClick={() => setCat(c)}
              className={`h-8 px-3.5 rounded-full text-xs font-semibold whitespace-nowrap transition ${selectedCat === c ? 'bg-amber-400 text-black' : 'bg-white text-stone-400 hover:text-stone-600'}`}>
              {c === 'all' ? 'All' : c}
            </button>
          ))}
        </div>
        ...
        {filtered.map(item => {
          if (mode === 'bundle') return (
            <button key={item.id} onClick={() => doAdd(item)} className="bg-white rounded-3xl p-4 text-left active:scale-[.97] transition-transform">
              <div className="w-full aspect-[4/3] bg-stone-100 rounded-2xl mb-3 flex items-center justify-center text-3xl opacity-20">🎁</div>
